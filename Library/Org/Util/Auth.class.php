...
class Auth
{
    ...
    public function getGroups($uid)
    {
        ...
        $user_groups = M()->getAll("SELECT g.uid,a.group_id,g.title,g.rules FROM {$this->_config['AUTH_GROUP_ACCESS']} a INNER JOIN {$this->_config['AUTH_GROUP']} g on a.group_id=g.id  WHERE ( a.uid=:uid and g.status=:status )", ['uid' => $uid, 'status' => '1']);
    ...
    protected function getAuthList($uid, $type)
    {
        ...
        foreach ($rules as $rule) {
            ////拼接 name
            $rule['name'] = $rule['m'] . '/' . $rule['c'] . '/' . $rule['a'] . '/' . $rule['data'];
    ...
    protected function getUserInfo($uid)
    {
        ...
        if (!isset($userinfo[$uid])) {
            $rules = M()->getOnce("SELECT * FROM {$this->_config['AUTH_USER']} WHERE uid =:id", ['uid' => $uid]);
        }
        return $userinfo[$uid];
    }

...
namespace Library\View\Template;
...
class Template
{
     ...
     * 加载主模板并缓存
     * @access public
     * @param string $templateFile 模板文件 或者模版内容字符
     * @param string $prefix 模板标识前缀
     * @return string
    ...
    protected function compiler($tmplContent)
    {
        ...
        // 添加安全代码
        $tmplContent = '<?php if (!defined(\'DAOGE_PATH\')) exit();?>' . $tmplContent;
        // 优化生成的php代码
        $tmplContent = str_replace('?><?php', '', $tmplContent);
        // 模版编译过滤标签
        $tmplContent = $this->templateContentReplace($tmplContent);
    ...
    public function parseVar($varStr)
    {
        ...
        if (!empty($varStr)) {
            ...
            if ('DAOGE.' == substr($var, 0, 6)) {
                // 所有以DAOGE.打头的以特殊变量对待 无需模板赋值就可以输出
                $name = $this->parseDAOGEVar($var);
    ...
    /**
     * 特殊模板变量解析
     * 格式 以 $DAOGE. 打头的变量属于特殊模板变量
     * @access public
     * @param string $varStr  变量字符串
     * @return string
     */
    public function parseDAOGEVar($varStr)
    {
        $vars     = explode('.', $varStr);
